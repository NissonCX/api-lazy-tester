...
# AI 接口测试脚本
# 用法: ./test-api.sh -m METHOD -u FULL_URL -t TOKEN [-b BODY] [-H "Header: Value"] [-s FORMAT]
#
# 示例:
...
#   ./test-api.sh -m POST -u "http://your-api-host:port/api/xxx" -t "Bearer xxx" -H "X-Request-ID: test" -b '{}'
...
show_usage() {
  echo "用法: $0 -m METHOD -u FULL_URL -t TOKEN [-b BODY] [-H 'Header: Value'] [-s FORMAT]"
  echo ""
  echo "参数:"
  ...
  echo "  $0 -m POST -u 'http://localhost:8080/api/login' -t 'Bearer xxx' -b '{\"username\":\"test\"}'"
  echo "  $0 -m GET -u 'http://localhost:8080/api/user/1' -t 'Bearer xxx'"
  exit 0
}

# 解析参数
while getopts "m:u:t:b:H:s:h" opt; do
  case $opt in
    m) METHOD="$OPTARG" ;;
    u) URL="$OPTARG" ;;
    t) TOKEN="$OPTARG" ;;
...
# 构建 curl 命令
CURL_CMD="curl -s -w '\n---STAT---\n%{http_code}\n%{time_total}' -X $METHOD '$URL' -H 'Content-Type: application/json' -H 'Authorization: $TOKEN' $EXTRA_HEADERS"
...
if [ $EXIT_CODE -ne 0 ]; then
  HTTP_CODE="000"
  TIME_TOTAL="0"
  RESPONSE_BODY="{\"error\": \"curl failed with exit code $EXIT_CODE\"}"
fi
